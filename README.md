...
<img src="/image/kubearmor.png" width="20%" alt="kubearmor Logo" /><img src="/image/kubescape.png" width="20%" alt="kubescape Logo" /><img src="/image/tracee.png" width="30%" alt="tracee Logo" /></p>
...
Let's measure with tracing policies
```shell
kubectl delete -f opentelemetry/loadtest_job.yaml -n otel-demo
kubect apply -k tetragon/policices
```

let's wait 30 min and run the load test:
```shell
kubectl apply -f opentelemetry/loadtest_job.yaml -n otel-demo
```
...
Let's measure with events produced with only process
```shell

kubectl delete -f opentelemetry/loadtest_job.yaml -n otel-demo
kubectl annotate ns otel-demo kubearmor-visibility=process--overwrite
kubectl annotate ns goat-app kubearmor-visibility=process --overwrite
kubectl annotate ns unguard kubearmor-visibility=process --overwrite
kubectl annotate ns default kubearmor-visibility=process --overwrite
```
wait 30 min and launch the load test
```shell

kubectl apply -f opentelemetry/loadtest_job.yaml -n otel-demo
```
Let's measure with capabilities events and policies 
```shell

kubectl delete -f opentelemetry/loadtest_job.yaml -n otel-demo
kubectl apply -k kubearmor/policies
```
wait 30 min andlaunch the load test
```shell
wait 30 min and
kubectl apply -f opentelemetry/loadtest_job.yaml -n otel-demo
```
Let's measure with events produced with only process and capabilities
```shell

kubectl delete -f opentelemetry/loadtest_job.yaml -n otel-demo
kubectl delete -k kubearmor/policies
kubectl annotate ns otel-demo kubearmor-visibility=process,capabilities --overwrite
kubectl annotate ns goat-app kubearmor-visibility=process,capabilities --overwrite
kubectl annotate ns unguard kubearmor-visibility=process,capabilities --overwrite
kubectl annotate ns default kubearmor-visibility=process,capabilities --overwrite
```
wait 30 min and launch the load test
```shell
...
kubectl apply -f opentelemetry/loadtest_job.yaml -n otel-demo
```
Let's measure with events produced with only capabilities , network & process 
```shell

kubectl delete -f opentelemetry/loadtest_job.yaml -n otel-demo
kubectl delete -k kubearmor/policies
kubectl annotate ns otel-demo kubearmor-visibility=network,capabilities,process --overwrite
kubectl annotate ns goat-app kubearmor-visibility=network,capabilities,process --overwrite
kubectl annotate ns unguard kubearmor-visibility=network,capabilities,process --overwrite
kubectl annotate ns default kubearmor-visibility=network,capabilities,process --overwrite
```
wait 30 min and launch the load test
```shell
...
kubectl apply -f opentelemetry/loadtest_job.yaml -n otel-demo
```
Let's measure with events produced with only capabilities , network ,process & file
```shell

kubectl delete -f opentelemetry/loadtest_job.yaml -n otel-demo
kubectl delete -k kubearmor/policies
kubectl annotate ns otel-demo kubearmor-visibility=network,capabilities,process,file --overwrite
kubectl annotate ns goat-app kubearmor-visibility=network,capabilities,process,file --overwrite
kubectl annotate ns unguard kubearmor-visibility=network,capabilities,process,file --overwrite
kubectl annotate ns default kubearmor-visibility=network,capabilities,process,file --overwrite
```
wait 30 min and launch the load test
```shell
...
kubectl apply -f opentelemetry/loadtest_job.yaml -n otel-demo
```
### 6. Deploy the  environment with Kubescape
